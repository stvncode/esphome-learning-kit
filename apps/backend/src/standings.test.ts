...
describe("generateCode", () => {
  ...
  test("is deterministic for injected bytes and avoids ambiguous chars", () => {
    expect(generateCode(new Uint8Array([0, 1, 2, 3, 4, 5]))).toBe("ABCDEF");
    expect(generateCode()).not.toMatch(/[OIL01]/); // ambiguous chars excluded by construction
  });
});
