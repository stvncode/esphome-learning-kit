...
export function Sandbox() {
  ...
  const runSimulation = useCallback(() => {
    ...
    if (inputNodes.length === 0) {
      toast.error("Add an input node (Button, Motion, etc.) to start simulation")
      return
    }
          ...
          setNodes((prev) =>
            prev.map((n) => {
              if (n.id === nodeId) {
                ...
                if (n.type === "delay") return { ...n, data: { ...n.data, isActive: true } }
                if (n.type === "trigger") return { ...n, data: { ...n.data, isActive: true } }
                if (n.type === "action") return { ...n, data: { ...n.data, isActive: true } }
              }
              return n
            })
          )
      ...
      setNodes((prev) =>
        prev.map((n) => ({
          ...
          data: {
            ...n.data,
            isActive: false,
            isOn: n.type === "light" ? false : undefined,
          },
        }))
      )
...
`
  ...
  const saveProject = useCallback(() => {
    ...
    localStorage.setItem("sandbox-projects", JSON.stringify(updated))
        ...
        <div className="flex items-center gap-4">
          <div>
            <div className="mb-1 flex items-center gap-2">
              <Badge className="bg-gradient-to-r from-violet-500/20 to-fuchsia-500/20 text-violet-400">
                Sandbox
              </Badge>
            </div>
            <Input
              ...
              className="h-8 w-48 border-none bg-transparent p-0 text-2xl font-bold focus-visible:ring-0"
            />
          </div>
        </div>
